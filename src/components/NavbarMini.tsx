    ...
    <div className=" p-4 bg-gradient-to-r  ">
      ...
      {open && (
        ...
        <div className="absolute left-0 top-2 w-full h-[calc(100vh-6rem)] flex flex-col gap-8 items-center justify-center  z-10 bg-black">
          {links.map((item) => (
            <Link className="relative text-2xl text-gray-400 font-bold no-underline px-3 py-2 transition hover:text-yellow-300 font-serif	" href={item.url} key={item.id} onClick={() => setOpen(false)}>
              {item.title}
